...
mitre_tree.py
=============
Upgraded Vectorless RAG (v3) using MITRE ATT&CK structured tree traversal
with LLM-generated summaries, Ollama llama3.1:8b selection, completeness checks,
and coupled intent-entity resolution fallback routing.
"""
...
"""
    ...
    def context_for_query(self, query: str, top_k: int = 5) -> str:
        ...
        1. Technique to Groups (intent-fallback)
        2. Mitigation Lookup (intent-fallback)
        3. Exact ID / Name Lookup
        4. Threat Group Aliases and Relationships
        5. Semantic query selection via LLM node selection + Completeness expansion.
        """
        query_lower = query.lower()
        ...
        if is_tech_to_groups:
            ...
            if tid:
                ...
                # FALLBACK to Tier 2 to resolve technique semantically, then return groups
        ...
        if is_mitigation:
            ...
            if tid:
                ...
                # FALLBACK to Tier 2 to resolve technique semantically, then return mitigations
        ...
        if matched_group:
            # Check if it's a Group Info query
            ...
            if is_info_query:
                ...
                # Fetch raw details of top 5 associated techniques
