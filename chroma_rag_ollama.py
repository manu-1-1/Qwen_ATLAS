...
def get_ollama_embeddings(texts: list) -> list:
    """Helper to query the local Ollama instance for embeddings of a list of texts."""
    if not texts:
        return []
    data = {
...
col    = client.get_or_create_collection("attck_objects_ollama",
...
# Automatically ingest if empty
if col.count() == 0:
    print("ChromaDB Ollama collection is empty. Starting ingestion...")
    ingest_techniques()
    ingest_groups()
    ingest_mitigations()
    print("ChromaDB Ollama collection ingestion complete.")
...
def lookup_group(query, top_k=5):
    ...
    if is_enumeration and requested_tactic is None:
        ...
        doc_vecs = get_ollama_embeddings(result["documents"])
...
if __name__ == "__main__":
    print("ChromaDB Ollama fully initialized and collection contains", col.count(), "items.")
